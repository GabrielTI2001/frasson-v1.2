...
class ContratoServicosView(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        search = self.request.query_params.get('search', None)
        subquery = Cobrancas_Pipefy.objects.filter(contrato_id=OuterRef('id')).values('contrato_id').annotate(
            num_cobrancas=Count('id')).values('num_cobrancas')[:1]
        subquery2 = Cobrancas_Pipefy.objects.filter(contrato_id=OuterRef('id'), phase_id=317532039).values('contrato_id').annotate(
            num_cobrancas=Count('id')).values('num_cobrancas')[:1]
        subquery3 = Contratos_Pagamentos.objects.filter(contrato_id=OuterRef('id')).values('contrato_id').annotate(
...
class ContratosPagamentosView(viewsets.ModelViewSet):
    ...
    queryset = Contratos_Pagamentos.objects.all()
