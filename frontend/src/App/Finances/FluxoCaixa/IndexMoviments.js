...
import { columnsMovimentacao} from "../Data";
...
const InitData = {
    ...
    'urlview':'/finances/financial/', 'title': 'Movimentações Financeiras'
}

const IndexMovimentacoes = () => {
    ...
    const onClick = (data, type) =>{
        if (type === 'edit'){
            ...
            setModalDelete({show:true, link: `${process.env.REACT_APP_API_URL}/finances/moviments/${data.id}/`})
        }
    }
    ...
    return (
        ...
        <FloatButton title='Nova Movimentação' onClick={() =>{setShowModal({show:true, type:'add'})}}/>
        <ModalForm show={showmodal.show} onClose={() => setShowModal({show:false})} 
            title={showmodal.type === 'add' ? 'Adicionar' : 'Editar'+' Movimentação'}
        >
            {showmodal.type === 'add' 
  ...
  export default IndexMovimentacoes;
