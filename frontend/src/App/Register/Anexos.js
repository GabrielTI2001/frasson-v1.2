...
export const Anexos = ({record, analisesolo, other}) => {
  const user = JSON.parse(localStorage.getItem('user'))
  const [formData, setFormData] = useState({analise_solo:analisesolo ? record.id : null, uploaded_by:user.id});
  ...
  if (formData.file && !isUploading){
    ...
    api.post('register/anexos/', formDataToSend, {headers: {Authorization: `Bearer ${token}`}, 
  ...
  useEffect(() =>{
    const getdata = async () =>{
      if (!anexos){
        const param = analisesolo ? 'analisesolo' : other ? 'other' : 'other2'
        const status = await HandleSearch('', 'register/anexos',(data) => {setAnexos(data)}, `?${param}=${record.id}`)
        if (status === 401){
          RedirectToLogin(navigate)
        }
      }
    ...
    <div className='mt-2 pe-2'>
        {anexos ? anexos.map(a => 
            ...
            >
                ...
                onClick={() => {setModaldel({show:true, link:`${process.env.REACT_APP_API_URL}/register/anexos/${a.id}/`})}}>
