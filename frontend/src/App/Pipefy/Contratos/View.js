...
import { Button, Col, Row, Card, OverlayTrigger, Tooltip, Modal, CloseButton, Table } from 'react-bootstrap';
...
const ViewContrato = () => {
    ...
    <ol className="breadcrumb breadcrumb-alt fs-xs">
        <li className="breadcrumb-item fw-bold">
            <Link className="link-fx text-primary" to={'/pipefy/contracts'}>Contratos</Link>
        </li>
    ...
    <Row className='mt-3 mb-2' xl={2} sm={2} xs={1}>
        <Col className='d-flex flex-column'>
            ...
            <Row className='mb-2'>
                <Col>
                    {card.str_servicos.split(', ').map(s =>
                        <span className='fs--2 badge bg-secondary fw-normal' key={s}>{s}</span>
                    )}
                </Col>
            </Row>
            ...
            <Row className='gy-1'>
                ...
                <Col className='mt-0' xl={6} xs={12}>
                    ...
                    <span className='my-1'>
                        {card.valor_gai ? Number(card.valor_gai).toLocaleString('pt-br', {style:'currency', currency:'BRL'}) : '-'}
                    </span>
                </Col>
                }
                {card.percentual_gc &&
                    <Col className='mt-0' xl={6} xs={12}>
                        ...
                        <span className='my-1'>
                            {card.percentual_gc ? Number(card.percentual_gc).toLocaleString('pt-br', {maximumFractionDigits:2}) : '-'}
                        </span>
                    </Col>
                }
                <Col xl={12} xs={12}> 
                    ...
                    <span className='my-1'>
                        {card.detalhes_negociacao || '-'}
                    </span>
                </Col>
                {card.etapas.length > 0 && card.pdf_contrato && (card.valor_gai || card.percentual_gc) &&
                    <div><Link className='btn btn-sm btn-warning py-0 fs--2' to={card.pdf_contrato}>PDF Contrato</Link></div>
                }
                <div><Link className='text-primary fw-bold fs--2' to={card.url_record} target='__blank'>Clique Aqui</Link> para ver o registro no Pipefy.</div>
        ...
        <Col className='d-flex flex-column'>
                    ...
                    >
                        ...
                        <td className="text-center text-middle fs--2">
                            ...
                            <FontAwesomeIcon icon={faTrash} 
                                onClick={() => setModal({show:true, link:`${process.env.REACT_APP_API_URL}/pipefy/contratos-pagamentos/${e.id}/`})}
                            />
                        </td>
                    </tr>
                )} 
            ...
            <Table responsive className="mt-1">
                ...
                {card && card.list_processos.map(p => 
                    <tr key={p.id} style={{cursor:'pointer'}} onClick={() => window.open(p.url,'_blank') } 
                        className={`${theme === 'light' ? 'hover-table-light': 'hover-table-dark'} py-0`}
                    >
                ...
                {card.etapas.length > 0 && card.pdf_contrato && (card.valor_gai || card.percentual_gc) ?
                    <div className='mt-1'>
                        <Link className='btn btn-sm btn-success py-0 fs--2' 
                            to={`${process.env.REACT_APP_API_URL}/pipefy/contracts-pdf/${card.id}`}
