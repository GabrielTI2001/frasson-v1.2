...
export const Anexos = ({record, licenca, other}) => {
  const user = JSON.parse(localStorage.getItem('user'))
  const [formData, setFormData] = useState({licenca:licenca ? record.id : null, uploaded_by:user.id});
  ...
  if (formData.file && !isUploading){
    ...
    api.post('licenses/anexos/', formDataToSend, {headers: {Authorization: `Bearer ${token}`}, 
  ...
  useEffect(() =>{
    const getdata = async () =>{
      if (!anexos){
        const param = licenca ? 'licenca' : other ? 'other' : 'other2'
        const status = await HandleSearch('', 'licenses/anexos',(data) => {setAnexos(data)}, `?${param}=${record.id}`)
        if (status === 401){
          RedirectToLogin(navigate)
        }
      }
    ...
    <div className='mt-2 pe-2'>
        {anexos ? anexos.map(a => 
            ...
            >
                ...
                onClick={() => {setModaldel({show:true, link:`${process.env.REACT_APP_API_URL}/licenses/anexos/${a.id}/`})}}>
